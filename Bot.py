...
def echo_all(message):
    if message.text == '/help':
        bot.reply_to(message, 'Попробуй команду /reg')
...
def callback_worker(call):
    if call.data == "yes":
        bot.send_message(call.message.chat.id, "Приятно познакомиться! Запишу тебя на курсы в Mediasoft!")
    elif call.data == "no":
        bot.send_message(call.message.chat.id, "Видимо, ты такой же бесполезный как и я =(")
